...
import { canSubmitFeedback } from '../utils/permissions'
...
export function FeedbackManagementPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const [pageRes, sla] = await Promise.all([
        fetchFeedbacksPage({
          page,
          pageSize: PAGE_SIZE,
          sortCol,
  ...
  }, [page, sortCol, sortAsc, filters, t])
  ...
  return (
    <div className="flex-1 space-y-6 overflow-auto p-4 md:p-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[var(--text-heading)]">{t('feedbackMgmt.title')}</h1>
          <p className="text-sm text-[var(--text-muted)]">{t('feedbackMgmt.subtitle')}</p>
        </div>
        <Link to="/app">
          <Button type="button" variant="secondary">
            {t('feedbackMgmt.backToDashboard')}
          </Button>
        </Link>
      </div>
      ...
      <Card>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_minmax(240px,280px)]">
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            <div>
              <label htmlFor="fm_q" className={FILTER_LBL}>
            ...
            <div>
              <label htmlFor="fm_loc" className={FILTER_LBL}>
                {t('dashboard.locPh')}
              </label>
              <input
                id="fm_loc"
